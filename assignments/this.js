...
* 3. When we want to make a template for an object, we capitalize the name. The keys in that object will all have this.keyname, 
(as well as use a '=', not a ':', and they're offset by semicolons, not commas.)
Then, to make a new one, we make a new variable and assign it to a new object (by using the keyword "new"). 
We write a specific parameter that will pass into the parameter of the original object template. The 'this' (which is found in the original 
object, the template) points to the new object.
